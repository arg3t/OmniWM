...
final class OverviewMutationSession {
    ...
    private func synchronizeStructuralSelection(_ mutation: StructuralMutation) {
        ...
        if wmController.workspaceManager.activeLayoutKind(for: mutation.destinationWorkspaceId) == .stack,
           let engine = wmController.stackEngine
        {
            wmController.workspaceManager.withEngineMutationScope(in: mutation.destinationWorkspaceId) {
                engine.activate(mutation.selectedHandle.id, in: mutation.destinationWorkspaceId)
            }
        }
    }
